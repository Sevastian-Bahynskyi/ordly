...
export function ReviewSession({ initialItems, translationLanguage = 'ru' }: { initialItems: ReviewItem[]; translationLanguage?: 'ru' | 'en' | 'uk' }) {
  ...
  const mode = current ? reviewMode(current.reps) : 'recognition'
  const entry = current?.vocabulary_entries

  useEffect(() => {
    setFreshSentence(null)
    if (!current || mode !== 'cloze' || current.reps < 5 || current.reps % 5 !== 0) return
    fetch('/api/ai/review-sentence', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ entryId: current.entry_id, cycle: Math.floor(current.reps / 5) }) })
      .then((r) => r.ok ? r.json() : null)
      .then((d) => d?.sentence && setFreshSentence(d))
      .catch(() => {})
  }, [current?.id, current?.reps, current?.entry_id, mode])
  ...
  async function rate(rating: number) {
    ...
    const res = await fetch('/api/review/rate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cardId: current.id, rating, answerResult: result }) })
    if (res.ok) {
      setItems((x) => x.slice(1)); setCompleted((x) => x + 1); setAnswer(''); setRevealed(false); setResult(null); setFreshSentence(null)
    }
  ...
  return <>
    ...
    <section className={`flash-card ${revealed ? 'revealed' : ''}`}>
      <div className="card-topline"><span className="prompt-type">{mode === 'recognition' ? `Danish → ${languageLabel}` : mode === 'production' ? `${languageLabel} → Danish` : 'Fill the Danish word'}</span><span className="card-status">{entry.learning_status}</span></div>
      ...
      <form onSubmit={submitAnswer} className="answer-form">
        ...
        <div className={`answer-input-wrap ${revealed ? result || '' : ''}`}><input autoFocus disabled={revealed} value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder={mode === 'recognition' ? `Type the ${languageLabel} meaning…` : 'Type the Danish word…'} />{revealed && (result === 'incorrect' ? <X size={20}/> : <Check size={20}/>)}</div>
      ...
      {revealed && <div className="answer-reveal">
        <div className={`answer-verdict ${result}`}><strong>{result === 'correct' ? 'Correct' : result === 'mostly' ? 'Almost right' : 'Not quite'}</strong>{result === 'mostly' && <span>Close enough for recognition, but notice the difference.</span>}</div>
        <div className="correct-answer"><span>Correct answer</span><strong>{expected}</strong>{mode !== 'cloze' && entry.example_sentence && <p>{entry.example_sentence}<small>{entry.example_translation}</small></p>}</div>
    ...
    <div className="review-tip"><RotateCcw size={15}/> Sentences rotate over time so you learn the word, not one memorized example.</div>
  </>
}
