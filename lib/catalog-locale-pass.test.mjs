...
test('a Ukrainian reply is held to the Ukrainian check, so Russian filed as Ukrainian is refused', async () => {
  const { loadUkrainianCheckers } = await import('./ukrainian-dictionaries.ts')
  const spell = await loadUkrainianCheckers()
  const uk = (senses) => ({ lang: 'uk', generator: 'test', senses })
  const ukrainian = [
    { ...good[0], text: 'раз (випадок)' },
    { ...good[1], text: 'коридор', example_translation: 'Він стоїть у коридорі.' },
  ]
  assert.deepEqual(checkLocaleReply(work, uk(ukrainian), 'uk', spell), [])
  assert.deepEqual(checkLocaleReply(work, uk([ukrainian[0], { ...ukrainian[1], example_translation: 'Он стоит в коридоре.' }]), 'uk', spell),
    ['gang#2: example translation is not Ukrainian (Russian words: стоит, коридоре)'])
  assert.deepEqual(checkLocaleReply(work, uk(ukrainian), 'uk'), ['the Ukrainian check needs its dictionaries'], 'the check never silently skips')
})
