...
test('sparse Material offers the shorter session instead of fabricating exercises, or sends the learner to Review', async () => {
  ...
  assert.deepEqual(shortfall, { requestedMinutes: 20, availableMinutes: 1, reviewOnly: 0 })
  ...
  assert.deepEqual((await startPractice(empty.client, USER, { minutes: 10, acceptShorter: true })).shortfall, { requestedMinutes: 10, availableMinutes: 0, reviewOnly: 0 })
  assert.equal(empty.row(), null)
  assertReviewAndMaterialUntouched(sparse)
})

test('Material entered only by hand plans nothing and says it is left to Review, rather than inventing content', async () => {
  const manual = fixture({ cards: WORDS.slice(0, 4).map((word, index) => {
    const item = card(index, word)
    return { ...item, vocabulary_entries: { ...item.vocabulary_entries, unverified: true } }
  }) })
  const { shortfall } = await startPractice(manual.client, USER, { minutes: 10, acceptShorter: true })
  assert.deepEqual(shortfall, { requestedMinutes: 10, availableMinutes: 0, reviewOnly: 4 })
  assert.equal(manual.row(), null, 'nothing is saved')
  assertReviewAndMaterialUntouched(manual)
})
