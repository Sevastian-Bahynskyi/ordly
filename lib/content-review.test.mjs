...
test('a repair fixes only the part the reviewers named, per kind, or drops the item', async () => {
  ...
  const hint = { id: 'p:x', kind: 'pronunciation', lemma: 'stå op', entry_kind: 'phrase', pos: 'verb', meaning: { ru: 'вставать', en: 'to get up' }, ipa: '[ˈsdɔˀ] [ˈʌb]', pronunciation: 'сдо́ об', stressed_word: 'op' }
  ...
  const reply = JSON.stringify({ items: [
    ...
    { id: 'p:x', pronunciation: 'сдо о́б' },
    { id: 'e:y#1', drop: 'the Danish is ungrammatical (det nyhed)' },
  ] })
  const calls = []
  const found = await repairPass([
    { item: meaning, problems: ['uk: в → у'] }, { item: example, problems: ['uk genitive'] }, { item: hint, problems: ['stress on op'] }, { item: bad, problems: ['det dårlige nyhed'] },
  ...
  assert.deepEqual(found.get('p:x'), { pronunciation: 'сдо о́б' })
  ...
  const wrong = parseRepairs(JSON.stringify({ items: [{ id: 'p:x', translations: { en: 'a', ru: 'б', uk: 'в' } }, { id: 'm:x#1', meaning: { ru: 'x' } }] }), [hint, meaning])
