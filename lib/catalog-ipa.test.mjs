...
import { chooseIpa, kaikkiPartOfSpeech, parseKaikkiLine, selectCatalogIpa, selectIpaForPos } from './catalog-ipa.ts'
...
test('component transcriptions are not treated as complete word pronunciations', () => {
  assert.equal(chooseIpa([{ ipa: '[-ˌtæˀ]' }]), null)
  assert.equal(chooseIpa([{ ipa: '[ˈsemi-]' }]), null)
  assert.equal(selectIpaForPos([{ word: 'påtage', pos: 'verb', ipa: '[-ˌtæˀ]' }], 'verb'), null)
})

test('a complete Wiktionary reading replaces DDO component notation', () => {
  assert.deepEqual(
    selectCatalogIpa('[-ˌtæˀ]', [{ word: 'påtage', pos: 'verb', ipa: '[ˈpʰɔːˌtæˀ]' }], 'verb'),
    { ipa: '[ˈpʰɔːˌtæˀ]', source: 'wiktionary' },
  )
  assert.deepEqual(selectCatalogIpa('[-ˌtæˀ]', [], 'verb'), { ipa: null, source: null })
  assert.deepEqual(selectCatalogIpa('[ˈpʰɔːˌtæˀ]', [], 'verb'), {
    ipa: '[ˈpʰɔːˌtæˀ]',
    source: 'ddo',
  })
})
