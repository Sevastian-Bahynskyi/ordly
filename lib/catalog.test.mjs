...
import { addCatalogMeaning, candidateLemmas, catalogMerge, encounteredFormOf, findSavedCatalogWord, lookupCatalog, parseCatalogEntry, unlockedDraft } from './catalog.ts'
...
test('a phrase is looked up whole and never collapsed to one of its words', async () => {
  ...
  assert.deepEqual(reads, [{ table: 'word_catalog', kind: 'phrase', lemma: ['godt lide'] }], 'no register lookup, no single-word lemma')
})
