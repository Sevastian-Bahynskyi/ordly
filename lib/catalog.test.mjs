...
import { catalogLookupText, addCatalogMeaning, candidateLemmas, catalogMerge, encounteredFormOf, findSavedCatalogWord, lookupCatalog, parseCatalogEntry, unlockedDraft } from './catalog.ts'
...
test('adding a catalog meaning to a saved word unlocks it in place, never a second card', () => {
  ...
  // A word first saved from the Russian catalog, picked again by an English learner: the locked
  // copy is unlocked with the English wording, keeping its id.
  const english = unlockedDraft(parseCatalogEntry(BILINGUAL, 'en'), S1).senses
  const russianSaved = unlockedDraft(parseCatalogEntry(ENTRY, 'ru'), S2).senses
  const switched = addCatalogMeaning(russianSaved, english, S1)
  assert.deepEqual(activeSenses(switched.senses).map((sense) => [sense.id, sense.text]).sort(), [[S1, 'just (now)'], [S2, 'прямо']])

  // A meaning the learner removed earlier is restored, not appended under the same id again.
  const removed = saved.map((sense) => sense.id === S1 ? { ...sense, locked: false, removed_at: '2026-09-20T00:00:00Z' } : sense)
  const restored = addCatalogMeaning(removed, unlockedDraft(parseCatalogEntry(ENTRY, 'ru'), S1).senses, S1)
  assert.equal(restored.status, 'added')
  assert.equal(restored.senses.filter((sense) => sense.id === S1).length, 1)
  assert.equal(restored.senses.find((sense) => sense.id === S1).removed_at, null)
...
test('a phrase is looked up whole and never collapsed to one of its words', async () => {
  const reads = []
  const client = {
    from(table) {
      const filters = {}
      const query = {
        ...
        eq: (column, value) => { filters[column] = value; return query },
        in: (column, values) => { filters[column] = values; return query },
        then(resolve) { reads.push({ table, ...filters }); return Promise.resolve({ data: [], error: null }).then(resolve) },
      }
      return query
    },
  }
  assert.deepEqual(await lookupCatalog(client, 'godt lide', 'en'), { candidates: [], miss: 'phrase' })
  assert.deepEqual(reads, [
    { table: 'word_catalog', kind: 'phrase', lemma: ['godt lide'] },
    { table: 'word_catalog_form', kind: 'phrase', form_text: ['godt lide'] },
  ], 'no register lookup, no single-word lemma')
})

const STÅ_OP = {
  lemma: 'stå op', kind: 'phrase', freq_rank: null, pos: 'verb', gender: null, definite_singular: null, pronunciation: 'сдо óб', audio_path: null, example_sentence: null, example_translation: null,
  word_catalog_sense: [{ sense_id: '66666666-6666-4666-8666-666666666666', ordinal: 1, lang: 'en', text: 'to get up', pos: 'verb', gender: null, example: 'Jeg står op klokken syv.', example_translation: 'I get up at seven.' }],
  word_catalog_form: [
    { form_key: 'present', form_text: 'står op', gender: '' }, { form_key: 'present', form_text: 'står … op', gender: '' },
    { form_key: 'past', form_text: 'stod op', gender: '' }, { form_key: 'past', form_text: 'stod … op', gender: '' },
  ],
}

/** Phrase reads: a headword by lemma, a form through the phrase form table. */
function phraseClient(rows) {
  return {
    from(table) {
      const filters = {}
      const query = {
        ...
        in: (column, values) => { filters[column] = values; return query },
        then(resolve) {
          const data = table === 'word_catalog_form'
            ? rows.flatMap((row) => row.word_catalog_form.filter((form) => [filters.form_text].flat().includes(form.form_text)).map((form) => ({ lemma: row.lemma, form_key: form.form_key, form_text: form.form_text })))
            : rows.filter((row) => row.kind === filters.kind && filters.lemma.includes(row.lemma))
          return Promise.resolve({ data, error: null }).then(resolve)
        },
      }
      return query
...
test('an inflected or split phrase finds its headword, which is what gets saved', async () => {
  for (const typed of ['stod op', 'står … op', 'står ... op', 'Står...op', 'stå op']) {
    const { candidates, miss } = await lookupCatalog(phraseClient([STÅ_OP]), typed, 'en')
    assert.equal(miss, null, typed)
    assert.deepEqual(candidates.map((entry) => entry.lemma), ['stå op'], typed)
    assert.equal(unlockedDraft(candidates[0], candidates[0].senses[0].sense_id).danish, 'stå op')
  }
  const [entry] = (await lookupCatalog(phraseClient([STÅ_OP]), 'stod op', 'en')).candidates
  assert.deepEqual(encounteredFormOf(entry, 'stod op'), { text: 'stod op', verified: true, isHeadword: false })
  assert.deepEqual(encounteredFormOf(entry, 'står ... op'), { text: 'står … op', verified: true, isHeadword: false })
  assert.deepEqual(encounteredFormOf(entry, 'stå op'), { text: 'stå op', verified: true, isHeadword: true })
  assert.equal(catalogLookupText('står...op'), 'står … op')
  assert.equal((await lookupCatalog(phraseClient([STÅ_OP]), 'står ned', 'en')).miss, 'phrase')
})

test('a compound tense finds its phrase through the participle form, without claiming the auxiliary', async () => {
  const withParticiple = { ...STÅ_OP, word_catalog_form: [...STÅ_OP.word_catalog_form, { form_key: 'past_participle', form_text: 'stået op', gender: '' }] }
  for (const typed of ['er stået op', 'var stået op', 'har stået op']) {
    const { candidates } = await lookupCatalog(phraseClient([withParticiple]), typed, 'en')
    assert.deepEqual(candidates.map((entry) => entry.lemma), ['stå op'], typed)
  }
  const [entry] = (await lookupCatalog(phraseClient([withParticiple]), 'er stået op', 'en')).candidates
  assert.deepEqual(encounteredFormOf(entry, 'er stået op'), { text: 'er stået op', verified: false, isHeadword: false })
  assert.equal((await lookupCatalog(phraseClient([withParticiple]), 'er op', 'en')).miss, 'phrase', 'only an auxiliary before a participle form is set aside')
})

/** Material reads for the duplicate path; records every filter so the owner-scoped reads can be checked. */
function materialClient(rows, { fail = false } = {}) {
  return {
    from(table) {
      assert.equal(table, 'vocabulary_entries')
      const filters = {}
      const query = {
        ...
        in: (column, values) => { filters[column] = values; return query },
        then(resolve) {
          const data = rows.filter((row) => row.entry_kind === filters.entry_kind
            && (filters.catalog_lemma === undefined || row.catalog_lemma === filters.catalog_lemma)
            && (filters.danish === undefined || filters.danish.includes(row.danish)))
          return Promise.resolve(fail ? { data: null, error: { message: 'down' } } : { data, error: null }).then(resolve)
        },
      }
      return query
    },
...
const savedRow = (patch) => ({ id: 'e1', danish: 'gulv', entry_kind: 'word', catalog_lemma: null, updated_at: '2026-09-01T00:00:00Z', senses: [{ id: 'm1', text: 'пол', pos: 'noun', gender: 'et', note: null, example: null, example_translation: null, source: 'user', coverage: { recognized: 3, produced: 1, last_seen: '2026-09-01T00:00:00.000Z' }, created_at: '2026-09-01T00:00:00Z', removed_at: null }], ...patch })
const gulvPick = { lemma: 'gulv', senseId: '93c644ae-08f2-5a1c-ac53-75bf47a66274', forms: ['gulv', 'gulvet', 'gulve', 'gulvene'] }

test('a different form of an already saved word finds that word instead of creating a second one', async () => {
  assert.equal((await findSavedCatalogWord(materialClient([savedRow({})]), gulvPick)).id, 'e1', 'saved as the headword')
  assert.equal((await findSavedCatalogWord(materialClient([savedRow({ danish: 'gulvet' })]), gulvPick)).id, 'e1', 'saved manually under the encountered form')
  assert.equal((await findSavedCatalogWord(materialClient([savedRow({ id: 'e2', danish: 'floor-ish', catalog_lemma: 'gulv' }), savedRow({})]), gulvPick)).id, 'e2', 'a catalog-backed copy wins')
  assert.equal(await findSavedCatalogWord(materialClient([savedRow({ entry_kind: 'sentence', danish: 'gulv' })]), gulvPick), null, 'a sentence is not the word')
  assert.equal(await findSavedCatalogWord(materialClient([], { fail: true }), gulvPick), 'error', 'a failed read never reads as "not saved"')
})

test('adding the English meaning to a Russian manual word keeps its meaning, id and coverage', async () => {
  const found = await findSavedCatalogWord(materialClient([savedRow({})]), gulvPick)
  const english = unlockedDraft(parseCatalogEntry(GULV, 'en'), gulvPick.senseId)
  const merged = catalogMerge(found.senses, { senses: english.senses, example_sentence: 'Bogen ligger på gulvet.', example_translation: 'The book is on the floor.' }, gulvPick)
  assert.equal(merged.status, 'added')
  assert.deepEqual(merged.senses[0], found.senses[0], 'the saved meaning and its Review-backed coverage are untouched')
  assert.deepEqual(merged.senses.map((sense) => [sense.id, sense.text]), [['m1', 'пол'], [gulvPick.senseId, 'floor']])
  assert.equal(merged.senses[1].example, 'Bogen ligger på gulvet.', 'a non-primary meaning carries its own example')
})
