...
test('exports words and phrases with escaped history, but excludes sentences', () => {
  const csv = buildMaterialCsv({
    ...
    practiceAttempts: [{
      ...
      level: 'A2',
      practiceSessionId: '2026-09-22-a2-5-adaptive-v1',
      exerciseId: 'a2-choice-noedvendig',
    }],
  ...
  assert.match(csv, /""source"":""ordly-exercises""/)
  assert.match(csv, /""level"":""A2""/)
  assert.match(csv, /""exercise_id"":""a2-choice-noedvendig""/)
})


test('exports labeled forms and form meanings for a saved headword', () => {
  const csv = buildMaterialCsv({
    entries: [{ ...entry, danish: 'lille', translation: 'маленький' }], cards: [], reviewLogs: [], practiceAttempts: [],
    forms: [{ user_id: 'user-1', entry_id: 'entry-1', form_key: 'plural', form_text: 'små', gender: '', source: 'cor', gloss: 'маленькие', updated_at: '2026-09-24T00:00:00Z' }],
  })
  assert.match(csv, /ordly-material-v2/)
  assert.match(csv, /Plural: små \(маленькие\)/)
  assert.match(csv, /forms_json/)
  assert.match(csv, /""gloss"":""маленькие""/)
})
