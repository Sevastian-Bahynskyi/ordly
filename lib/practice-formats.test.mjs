...
test('a dialogue is offered only for a saved headword, and only in a language its situation exists in', () => {
  ...
  const ukrainian = dialogueTasks(saved, 'uk', 's')
  assert.deepEqual(ukrainian.map((task) => task.danish).sort(), ['gerne', 'måske'], 'every pilot situation exists in Ukrainian (issue #24)')
  assert.ok(ukrainian.every((task) => task.support && /[іїєґ]/i.test(task.support)))
