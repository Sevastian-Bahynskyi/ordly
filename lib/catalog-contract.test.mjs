...
test('component IPA cannot cross the catalog fact boundary', () => {
  assert.equal(parseCatalogFact({ ...fact, ipa: '[-ˌtæˀ]' }), null)
  assert.equal(parseCatalogFact({ ...fact, ipa: '[ˈsemi-]' }), null)
})
...
test('the prompt carries the source facts unchanged and mirrors the hard gate', () => {
  ...
  assert.match(prompt, /exactly 1 object in exactly the input order/)
