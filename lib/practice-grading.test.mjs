...
test('practice recall ignores removed synonym links', async () => {
  const f = fixture({ links: [link] })
  await actOnPractice(f.client, 'user', f.action('answer', { answer: 'тяжело' }))
  assert.equal(f.state().session.current.result, 'ungraded')
  assert.equal(f.state().session.aiCalls, 1, 'semantic grading is still available')
})
