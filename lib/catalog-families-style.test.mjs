...
test('a subject pronoun right after a preposition is flagged', async () => {
  ...
  assert.deepEqual(subjectPronounAfterPreposition('Hvornår ved du det?'), [])
})
...
test('a finite particle verb cannot stay unbroken after a fronted adverbial', async () => {
  ...
  assert.equal(finitePhraseAfterFrontedAdverbial('Selvom kalenderen er fuld, har travlt jeg.', 'har travlt', 'have').length, 1)
  assert.deepEqual(finitePhraseAfterFrontedAdverbial('Jeg har travlt, selvom kalenderen er fuld.', 'har travlt', 'have'), [])
})

test('a sentence adverb between the pronoun subject and the verb is flagged', async () => {
  const { adverbBeforeVerbInMainClause } = await import('./catalog-families-style.ts')
  assert.equal(adverbBeforeVerbInMainClause('Jeg aldrig spiller tennis.').length, 1)
  assert.deepEqual(adverbBeforeVerbInMainClause('Jeg spiller aldrig tennis.'), [])
  assert.deepEqual(adverbBeforeVerbInMainClause('Aldrig har jeg set det.'), [])
})
