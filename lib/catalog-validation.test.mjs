...
test('pronunciation uses combining stress marks rather than ASCII apostrophes', async () => {
  const row = wordRow()
  row.pronunciation = "го'л"
  const result = await validateCatalogBatch([wordFact()], [row], sources)
  assert.ok(codes(result).includes('pronunciation_invalid_script'))
})
...
test('the COR-backed catalog lemma is not rejected because the spelling dictionary lacks it', async () => {
  const fact = wordFact('skamskyde')
  const row = wordRow('skamskyde')
  row.senses[0].example = 'Jægeren kom til at skamskyde dyret.'
  const result = await validateCatalogBatch([fact], [row], {
    ...sources,
    findMisspellings: async () => ['skamskyde'],
  })
  assert.deepEqual(codes(result), [])
})
