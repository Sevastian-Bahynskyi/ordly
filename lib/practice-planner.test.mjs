...
test('how much Practice is available does not depend on the seed, so the shorter session matches its offer', () => {
  const items = Array.from({ length: 6 }, (_, i) => item(`w${i}`))
  const seconds = ['a', 'b', 'c', 'd'].map((seed) => queueSeconds(planPractice({ ...base, items, seed, targetMinutes: 30 }).queue))
  assert.equal(new Set(seconds).size, 1, seconds.join())
})
