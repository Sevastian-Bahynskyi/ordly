...
test('a phrase is gapped in a verified form, and its other forms grade as the wrong form', () => {
  assert.deepEqual(findInSentence('Hun stod op klokken syv.', 'stå op', ['står op', 'stod op', 'står … op']), { surface: 'stod op', gapped: 'Hun _____ klokken syv.' })
  assert.equal(findInSentence('Hun stod tidligt op.', 'stå op', ['stod op', 'stod … op']), null, 'a split phrase is never one gap')
  assert.equal(findInSentence('Hun stod op.', 'stå op'), null, 'no verified forms: no guess')
  const target = item('op', {}, { danish: 'stå op', senses: [sense('op-s', 'to get up', { pos: 'verb' })], example_sentence: 'Hun stod op klokken syv.', example_translation: 'She got up at seven.' })
  const candidate = { item: target, entryId: 'op', sense: target.vocabulary_entries.senses[0], primary: true, targetKey: senseTargetKey('op', 'op-s') }
  const cloze = clozeTypedTask({ candidate, senses: [], distractors: [], newTarget: false, reps: 0, forms: ['stå op', 'står op', 'stod op', 'står … op'] })
  assert.equal(cloze.prompt, 'Hun _____ klokken syv.')
  assert.equal(gradePracticeAnswer(cloze, 'stod op').result, 'correct')
  assert.equal(gradePracticeAnswer(cloze, 'står op').feedback, 'wrong_form')
})
...
test('an entry entered by hand outside the catalog is never practised, not even as a distractor', () => {
  const manual = item('manual', { reps: 0, stability: 0, last_review: null }, { unverified: true, created_at: '2026-09-09T12:00:00Z' })
  const others = Array.from({ length: 8 }, (_, i) => item(`checked${i}`))
  const session = planPractice({ ...base, items: [manual, ...others] })
  assert.ok(session.queue.length > 0)
  assert.ok(!JSON.stringify(session.queue).includes('manual'), 'no task targets, shows or grades the manual entry')
})

test('Material made only of manual entries plans nothing, rather than inventing content', () => {
  const session = planPractice({ ...base, items: [item('manual', {}, { unverified: true })] })
