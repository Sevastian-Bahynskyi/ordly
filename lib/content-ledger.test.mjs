...
import { addCall, budgetCheck, formatReport, issueAllocation, openingFromRawLog, priceCall, serviceAllowed, serviceOf, summarize } from './content-ledger.ts'
...
test('a service the program excludes is refused; no list means all three', () => {
  const textOnly = { services: ['deepseek', 'translator'] }
  assert.equal(serviceAllowed(textOnly, 'deepseek.review-a'), true)
  assert.equal(serviceAllowed(textOnly, 'translator.da-uk'), true)
  assert.equal(serviceAllowed(textOnly, 'speech.tts'), false)
  assert.equal(serviceAllowed({}, 'speech.tts'), true)
})
