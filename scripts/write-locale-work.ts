 ...
 *
 *   pnpm exec tsx scripts/write-locale-work.ts --lang en
 *
...
const outDir = join('catalog', 'locale', 'work')
await mkdir(outDir, { recursive: true })
let total = 0
for (const name of (await readdir('catalog/out')).filter((file) => /^batch-\d+\.json$/.test(file)).sort()) {
  const rows: LocaleWorkRow[] = []
  for (const value of parseCatalogGeneratorText(await readFile(join('catalog/out', name), 'utf8'))) {
